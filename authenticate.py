...
from exceptions import CredentialsError, DeprecationError, ForgotError, LoginError, RegisterError, ResetError, \
    UpdateError
...
class Authenticate:
    ...
    def login(self, location: str = 'main', max_concurrent_users: Optional[int] = None,
              fields: dict = {'Form name': 'Login', 'Username': 'Username', 'Password': 'Password',
                              'Login': 'Login'}) -> tuple:
        """
        ...
        if not st.session_state['authentication_status']:
            ...
            if not st.session_state['authentication_status']:
                ...
                self.username = login_form.text_input(
                    'Username' if 'Username' not in fields else fields['Username']).lower()
    ...
    def register_user(self, location: str = 'main', preauthorization: bool = True, domains: Optional[list] = None,
        ...
        new_password = register_user_form.text_input('Password' if 'Password' not in fields else fields['Password'],
                                                     type='password')
    ...
    def update_user_details(self, username: str, location: str = 'main',
                            fields: dict = {'Form name': 'Update user details',
                                            'Field': 'Field',
                                            'Name': 'Name',
                                            'Email': 'Email',
                                            'New value': 'New value',
        ...
        new_value = update_user_details_form.text_input(
            'New value' if 'New value' not in fields else fields['New value'])
